...
@
The nodes do not come in a specific order. Access via a permutation vector is preferred. Thisallows different views onthe same data set. For example, \vref{tab:prspbt} uses a permutation by total time, and a selection (compare to \vref{tab:prSRREbt}).
...
<<fig=TRUE, label= sprof01collink, width=18, height=6>>=
...
par(oldpar)
@
The obvious message is that if seen by level, there are different structures. Profiling usually takes place in a framework. So at the base of the stacks, we find entries that are (almost) resistent. Then in general we have some few steps where the algorithm splits, and then we have the finer details. This can be identified on the stack level.
...
<<fig=TRUE, label= sprof02collink, width=18, height=6>>=
oldpar <- par(mfrow=c(1,3))
...
par(oldpar)
@
For a visual inspection, runs of the same node and level in the profile are easily perceived. For
...
You con re-run it, using your  \ircode{sprof} data by modifying this instruction:
...
plot(sprofadjNEL,  main="graph layout", cex.main=5)
...
#plot(nwsprofadj, label=rownames(sprofadj), edge.lwd=sprofadj)
