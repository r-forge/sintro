...
<<>>=
strx <- function(x, ...){
	str(x, max.level=2, vec.len=3, nchar.max=40, 
	list.len=12,
...
Stack information is first  The call graph is a second instance that is (re)constructed from the stack recording. The graph represents cumulated one-step information. Longer scale information containted in the stacks ist lost in the graph.
...
\begin{figure}[htbp]
\begin{center}
<<fig = TRUE, echo = TRUE, print = FALSE, label=nrunique, width=3, height=6>>= 
...
abline(h=3.5,col="yellow")
abline(h=12.5,col="yellow")
@
...
Nr of unique nodes by stack level: See \vref{ig:nrunique}. We will come to finer tools in \vref{sec:rle} but for the moment the rough information should suffice to take a decision, with a matter of taste as to go for two levels to cut off, or three. Level three leaves us to start with five roots on the next level in our example.

% floating figure end
Not so often, but a frequent phenomenon is to have some ``burn in'' or ``fade out''. To identify this, we need to look at the profile level. The indicator to check is to whether we have very low frequency stacks at the beginning or the end of our recording.
<<>>=
# reference
summary(sprof01$stacks$freq)
#
df <- data.frame(stack=sprof01$profiles$data, count=sprof01$stacks$refcount[sprof01$profiles$data])
xcutdata.frame(df, margin=10)
@
Here at least two recordings on either side are candidates to be off.
...
plot(sprof01adjNEL,  main="sprof01: graph layout example", 
	sub=sprof01$info$id, cex.main=2)
rm(sprof01adjNEL); 
# detach("package:graph") 
#! sorry. still needed by Rgraphviz -- clean up
@
...
prxt(sprof02$nodes,cut=FALSE,  
	caption="sprof02, before update",label="tab:sprof02info1")
<< label=sprof02Upd>>=
...
str(found)
# sas.factor(sprof03$stacks$nodes[!is.na(found)], levels=1: length(sprof03$nodes$name), labels=sprof03$nodes$name)
@
...
\todo{support na removal in rrle}
...
are organized hierarchically. On the root level, runs are just ordinary runs. On the next levels, runs have to be defined given (within) the previous runs. So we need a recursive version of \ircode{rle}, applied to the profile information.
...
reducing it to a summary, we keep the full information. Information is always kept on i
ts original level. 
...
plot(as(adjacency(sprof01),"graphNEL"),  
	main="graph layout sprof01", cex.main=2, sub=sprof01$info$id)
plot(as(adjacency(sprof02),"graphNEL"),  
	main="graph layout sprof02", cex.main=2, sub=sprof02$info$id)
par(oldpar)
@
...
@
\todo{cut top levels}
<<fig=TRUE, label=sprofadj04NEL, width=24, height=12>>=
library(graph)
a04<-adjacency(sprof04)
rnames <- rownames(a04)
rnames[1]<-"noop";rownames(a04) <- rnames; colnames(a04) <- rnames; 
plot(as(a04,"graphNEL"),  main="graph layout sprof01", cex.main=2)
@
...
#24
plot(sprofadjNEL,  main="sprof04: graph layout", cex.main=2)
#detach("package:graph")
...
#6
plot(sprofadjRag, main="sprof04: Graphviz dot layout", cex.main=2)
@
%agopen(graph, name, nodes, edges, kind = NULL, layout = TRUE,
%layoutType = "dot",
%attrs = list(), nodeAttrs = list(), edgeAttrs = list(),
...
#6
plot(sprofadjRag,"neato", main="sprof04: Graphviz neato layout", 
	cex.main=2)
@
\filledpagebreak
<<fig=TRUE, label=s04adjviztwopi,width=6, height=6>>=
#6
plot(sprofadjRag,"twopi", main="sprof04: Graphviz twopi layout")
@
\filledpagebreak
<<fig=TRUE, label=sprofadjvizcirco,width=6, height=6>>=
#6
plot(sprofadjRag,"circo", main="sprof04: Graphviz circo layout", cex.main=2)
@
\filledpagebreak
<<fig=TRUE, label=s04adjvizfdp,width=6, height=6>>=
#6
plot(sprofadjRag,"fdp", main="sprof04: Graphviz fdp layout", cex.main=2)
@
\index{Index01}
%:Sweave examples
%<<print=TRUE>>=
