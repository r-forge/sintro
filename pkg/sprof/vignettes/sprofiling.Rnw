...
@
Another notorious nuisance is create by shortcuts, that is aliases which maybe make programming more convenient, but hide stucture.
<<>>=
...
\caption{Sanitised nsodes by user defined class. User defined colours.}
...
attibutes, we can use an \R{} \code{data.frame} as provided by
...
<<fig = TRUE, echo = TRUE, print = FALSE, label=plotsprof, png=TRUE, width=16, height=12, include=FALSE>>= plot
#12 16
...
<<fig=TRUE, label=plotnwsprof03,width=6, height=6>>=
#6 6
plot_network_sprof(sprof03)
@
\filledpagebreak

<<fig=TRUE, label=nwsprof03,width=8, height=8>>=
