...
The preview, at this point, taking package \ircode{graph} as an example. \ircode{graph} on its side has an undocumented feature: it needs  \ircode{Rgraphviz} to handle graph attributes. We have to take two steps. 
...
Walkinf further down, we see other candidates such as \ircode{as.character} that may be avoidable in this problem.
...
See \vref{fig:sprofadj01NEL} for a comparison before and after trimming. The scaffold effect are removed from the picture on the right side. This cuts off tehe uninformative spine, and induces minor changes in the body of the graph. You can do additional trimming, if you want.
...
\caption{Sprof graph, after trimming, and after trimming and some sanitizing.}
...
The derived edge frequency is the first bit of information Implicitly, it can be used as weight in the graph placement routines. We make this explicit by giving a choice whether to use  it or not. Irrespective of this choice, we encode referenc counts as line width of the edges.
...
@
%:   as_graphNEL_sprof
This is a common routine for the \ircode{grpah} and \ircode{Rgraphviz} package.
<<>>=
...
plot_graphNEL_sprof <- function(graphNEL)
{
	...
	#title(main=paste0( "Rgraphviz/graph( ",main1, " )"))
	title(main=main1)
	#title(main="nodes time", sub=sprof04$info$id)
	#legend("bottomright",legend= c("node color: self.time, pwr=0.5", "node 
	#lwd: total.time, pwr=0.5"), bty="n")

	legend("topleft", 
		legend=class(graphNEL),
		title="plot_graphNEL_sprof class:"
		)
...
\end{center}
\end{figure}
Nodes marked by run length and run count: see \vref{fig:sprofadj04NEL02rle}.  This plot gives us a more informative  view, highlighting nodes by number of runs and avaerage run lenght and edges with overall time presence.
...
#6 6
sprof_ig_auto <- as_igraph_sprof(sprof, layout.circle)
plot(sprof_ig_auto, main=paste0("igraph circle layout\n", sprof$info$id))
@
\filledpagebreak
...
as_network_sprof <- function(sprof) {
	sprofadj02 <- adjacency(sprof)
	nwsprofadj <- as.network(sprofadj02)
	network.vertex.names(nwsprofadj) <- 
		rownames(sprofadj02) # not honoured by plot
	return(nwsprofadj)
}
<<>>=
plot_network_sprof <- function(nwsprof,mode = "fruchtermanreingold", ...) {
...
<<fig=TRUE, label=plotnwsprofadj,width=6, height=6>>=
#6 6
plot_network_sprof(sprof04)

<<fig=TRUE, label=nwsprofadj,width=8, height=8>>=
nwsprofadj <- as.network(sprofadj02) # names is not imported 
#network.vertex.names(nwsprofadj) <- rownames(sprofadj02) # not honoured by plot
plot(nwsprofadj, label=rownames(sprofadj02), main="sprof02: network layout", cex.main=2)
@
\filledpagebreak
...
plot(nwsprofadj, label=rownames(sprofadj02), main="sprof02: network layout", cex.main=2, edge.lwd=edge.lwd)
...
nwsprofadj <- as.network(sprofadj02) # names is not imported 
network.vertex.names(nwsprofadj) <- rownames(sprofadj02) # not honoured by plot
plot(nwsprofadj, label=rownames(sprofadj02), 
