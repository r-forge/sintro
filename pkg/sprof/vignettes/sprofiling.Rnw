...
@
The nodes do not come in a specific order. Access via a permutation vector is preferred. This allows different views on the same data set. For example, \vref{tab:prspbt} uses a permutation by total time, and a selection (compare to \vref{tab:prSRREbt}). The only difference is that we work on a ms base internally, whereas \R{} uses seconds as a base. \todo{introduce cpu clock cycle as a time base}
...
library(Rgraphviz)
sprofadjRag <- agopen(sprofadjNEL, name="Rprof Example")
<<fig=TRUE, label=s03adjvizdot, width=24, height=24>>=
plot(sprofadjRag, main="Graphviz dot layout", cex.main=5)
@
%agopen(graph, name, nodes, edges, kind = NULL, layout = TRUE,
%layoutType = "dot",
