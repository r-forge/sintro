...
@
The nodes do not come in a specific order. Access via an access vector is preferred. For example \vref{tab:prspbt} (compare to \vref{tab:prSRREbt}).
...
caption="splot result: by.total, total time > 0.004s", 
	label="tab:prspbt")
@
Common rearrangement as by total time and by self time are supplied by the display functions.
Plot, for example, currently gives a choice of four displays.
