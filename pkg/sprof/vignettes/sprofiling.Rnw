...
\todo{Can we calibrate times to CPU rate?}
\todo{introduce cpu clock cycle as a time base}
...
\todo{Move class 
attributes to package: by keyword, by package, by custom}
...
\todo{this is by node count. use by time}
...
If package \ircode{wordcloud} is installed, a different view is possible.
...
if (require(wordcloud)){
	wordcloud(sprof$nodes$name, freq = sprof$nodes$total.time,
		...
		ordered.colors=TRUE)
			...
			frame(); legend("center","package wordcloud \n is required for this plot")}
...
\todo{sprea colour on displayd part}
<<>>=
...
\todo{check and stabilize colour linking}
<< label= shownodes>>=
...
#12
shownodes(sprof01)
<<fig=TRUE, label= shownodes01rb, width=18, height=18>>=
#18
shownodes(sprof01, col=rainbow(62))
<<fig=TRUE, label= shownodes01heat, width=20, height=20>>=
#20
...
Before starting additional inspection, the data better be trimmed. At this point, it is a decision whether to adapt the timing information, or keep the original information. Since this decision does affect the structural information, it is not critical. But analysis is easier if unused nodes are eliminated.\todo{add ?? function}
<<>>=
sprof02 <- sprof01
basetrim <- 13
...
<<fig=TRUE, label= sprof02shownodes14, width=14, height=14>>=
#14
shownodes(sprof02)
<<fig=TRUE, label= sprof02shownodes18, width=18, height=18>>=
#18
shownodes(sprof02)
...
\todo{colours. recolour}
<<fig=TRUE, label= shownodes02, width=10, height=10>>=
#18
shownodes(sprof02)
@
...
<<fig=TRUE, label=sprofadjNEL, width=10, height=10>>=
#10
library(graph)
...
library(graph)
sprofadjNEL <- as(sprofadj,"graphNEL")
<<fig=TRUE, label=s03adjNEL, width=24, height=24>>=
#24
plot(sprofadjNEL,  main="graph layout", cex.main=2)
#detach("package:graph")
...
library(igraph)
sprofig <- graph.adjacency(sprofadj)
<<fig=TRUE, label=s03ig, width=24, height=24>>=
#plot(sprofig, main="igraph layout", cex.main=5)
plot(sprofig, main="igraph layout")
...
\filledpagebreak
\subsubsection{network Package}
<<fig=TRUE, label=s03nwsprofadj, width=24, height=24>>=
library(network)
nwsprofadj <- as.network(sprofadj) # names is not imported 
network.vertex.names(nwsprofadj) <- rownames(sprofadj) # not honoured by plot
...
@
\filledpagebreak
<<fig=TRUE, label=s03nwsprofadje, width=24, height=24>>=
edge.lwd<-sprofadj
...
@
\filledpagebreak
<<fig=TRUE, label=s03nwsprofadjecircle, width=24, height=24>>=
plot(nwsprofadj, label=rownames(sprofadj), 
main="network circle layout: \n trimmed data", 
mode="circle",
cex.main=2, edge.lwd=edge.lwd)
<<fig=TRUE, label=s03nwsprofadjefruchtermanreingold, width=24, height=24>>=
plot(nwsprofadj, label=rownames(sprofadj), 
...
%dot, neato, twopi, circo, and fdp.
\filledpagebreak
<<fig=TRUE, label=s03adjvizneato, width=24, height=24>>=
plot(sprofadjRag,"neato", main="Graphviz neto layout", cex.main=5)
@
\filledpagebreak
<<fig=TRUE, label=s03adjviztwopi, width=24, height=24>>=
plot(sprofadjRag,"twopi", main="Graphviz twopi layout", cex.main=5)
@
\filledpagebreak
<<fig=TRUE, label=sprofadjvizcirco, width=24, height=24>>=
...
@
\filledpagebreak
<<fig=TRUE, label=s03adjvizfdp, width=24, height=24>>=
plot(sprofadjRag,"fdp", main="Graphviz fdp layout", cex.main=5)
@
\index{Index01}
%:Sweave examples
