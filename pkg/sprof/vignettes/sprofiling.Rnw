...
<<>>=
strx <- function(x, ...){
	str(x, max.level=2, vec.len=3, nchar.max=40, list.len=6,
...
To run the vignette with a different profile, replace \ircode{sprof01} by your example. You still have the file for reference.
 d
The data contain identification information for reference. This will be used in 
...
irfunx{hplot}{plot\_nodes}, for example, currently gives a choice of four displays for nodes, and supports trimming by default. Our profile starts with 62 nodes. The defaults cut off 34 nodes as uninformative, either because they are to rare, or ubiquitous.
...
Information in the scatterplots may sometimes more accessible
...
\caption{Nodes by package: default colour selection}
\label{fig:sREplotnodes01colcb}
\end{center}
\end{figure}

See \vref{fig:sREplotnodes01colcb}.
...
prxt(sprof02$nodes, caption="sprof02, after update", 
...
\begin{figure}[htbp]
\begin{center}
<<fig = TRUE, echo = TRUE, print = FALSE, label=shownodes02, width=8, height=8>>= 
#8
shownodes(sprof02)
@
\caption{Nodes by stack and profile}
