...
\emph{URL:} \url{http://sintro. r-forge.r-project.org/}
...
\emph{Typeset}, with minor revisions: \today\  from SVN $Revision$ \svnInfoDate}  
...
<<fig=TRUE, label= shownodes02, width=18, height=18>>=
#18
shownodes(sprof02)
@
...
<<fig=TRUE, label=sprofadjNEL12, width=12, height=12>>=
plot(sprof02adjNEL,  main="graph layout example", cex.main=2)
rm(sprof02adjNEL); 
...
\section{Graph: regression example}
\label{sec:graph}
...
@
\section{Graph II: trimmed regression example}
\label{sec:graph}
%Packages used to run examples or
%tests conditionally (e.g. via if(require(pkgname))) should be listed in ÔSuggestsÕ or
%ÔEnhancesÕ.
