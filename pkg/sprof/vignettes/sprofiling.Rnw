...
{
main=paste0(main, "\n plotviz> Source:",deparse(substitute(x)))
plot(x=x,  y=y,
...
@
\todo{we could do smart smoothng of the stacks here}
...
plot_graphNEL_sprof <- function(graphNEL)
{
	...
	title(main=paste0(main1, " Rgraphviz/graph"))
...
<<fig=TRUE, label=plot04NEL, width=6, height=6>>=
...
<<fig=TRUE, label=sprofadj04NEL02, width=6, height=6>>=
library(graph)
...
fill_list <- heat.colors(12)[rkindex(-sprof04$nodes$self.time, pwr=0.5, maxindex=12, ties.method="min")]
names(fill_list) <- sprof04$nodes$name

#sprof04$nodes$total.time -> lwd
lwd_list  <-  rkindex(sprof04$nodes$total.time, pwr=0.5,maxindex=6, ties.method="min")
...
<<fig=TRUE, label=sprofadj04NEL02rle, width=6, height=6>>=
