...
\todo{add reference to R profiling info}
...
Given the sampling structure of the profiles, two aspect are common. The sampling picks up scafffoling functions with a hicht, nearly constant requency. And the sampling will pick up rare recordings that are near to detection range. The display functions hide these effects by default. In our example, about half of the nodes are cleared by this garbage collector.
...
plot(sprof01adjNEL,  main="graph layout example", cex.main=2)
rm(sprof01adjNEL); 
...
@
\todo{cut next level}
\todo{Implement. Currently handled on source=text level}
...
str(sumsprof01, max.level=2, width=70
)
@
...
\section{Graph}
\label{sec:graph}
Graph layout is a theme of its own. Proposals are readily available, as are their implementation. For some of them, there are \R{} interfaces or re-implementations in \R{}. Their usefulness in out context has to be explored, and the answers will vary with personal preferences. 
...
@
\filledpagebreak
\subsection{trimmed regression example}
...
#edge.lwd <- trunc(sprofadj/max(sprofadj)*10)+1
edge.lwd <- round(edge.lwd/max(edge.lwd)*12)
plot(nwsprofadj, label=rownames(sprofadj), main="network layout: trimmed data", cex.main=2, edge.lwd=edge.lwd)
@
\filledpagebreak
