...
to \vref{tab:prSRREbt}). One difference is that \ircode{sprof} works uses 
an event count as base, usually sampled by milliseconds (ms), whereas \R{} in general uses seconds as a base. Another difference is in two additional variables, teh number of runs \ircode{nr\_runs} and  the average run length \ircode{avg\_time}. These are discussed in \label{sec:rle}.
...
Here is a run-down of the previous stages of our example, using this plot.
\filledpagebreak
...
\ircode{layout.norm(layout, 
	xmin = NULL, xmax = NULL, ymin = NULL, ymax = NULL,
      	zmin = NULL, zmax = NULL)}
@
The output of \ircode{igraph} gives problems when rendered with Adobe on OS X Maverick. These examples have been moved to the \ircode{demo} section.
<<>>=
demo(,"sprof")
@
%\subsubsection{igraph package}
