...
%}
<<label=plotviz>>=
plotviz <- function(x, method="dot", main=NULL, sub=NULL,...)
{
	xid <- deparse(substitute(x))
	xsubid <- NULL
	...
	if (is.null(main)) 
		main <-paste0("plotviz( ", xid ,", ", method, " )\n", xsubid) else
		main=paste0(main, "\n plotviz( ", xid ,", ", method, " )\n", xsubid)
...
#	title(sub = paste0(sub, " ", as.character(class(x)))	)
	...
	legend("topleft", 
		legend=c(paste0("class: ",class(x)), paste0("method: ",method)),
		bg="#FFFFE040",
		seg.len=0
		)#"lightyellow" =#FFFFE0
}
@
To make sure we do not depend onpackages collected along the way,
...
nodeclass[sprof01$nodes$name %in% c( "summay" ,"summay.lm")] <- "x_summary"
...
Note: trimming may be supported by the graph packages.
...
nodeclass[sprof04$nodes$name %in% c( "summay" ,"summay.lm")] <- sprof04$nodes$icol <-as.factor(nodeclass)
@
now gives
...
@
See \vref{tab:pramt1}: Marginal statistics on nodes by run length.
%<<>>=
...
This function is not included in the package to avoid dependency of \ircode{sprof} on \ircode{graph} and other graph packages.
<<>>=
library(graph)
search() 
<<>>=
...
<<>>=
plot_graphNEL_sprof <- function(graphNEL, mode="dot", main=NULL, sub=NULL,...)
{
	...
	if (!is.null(sub)) sub <- as.character(sub)
	if (is.null(main)) 
		main <-paste0("plot_graphNEL_sprof( ",main1 ,", ", y, " )\n", xsubid) else
		main=paste0(main, "\n plot_graphNEL_sprof( ", main1 ,", ", y, " )\n", xsubid)

	title(main=main1, sub=sub)

	legend("topleft", 
		legend=paste0("class: ",class(graphNEL)),
		bg="#FFFFE040",
		seg.len=0
		)
...
<<fig=TRUE, label=sprofadj04NEL02, width=6, height=6, include=FALSE>>=
library(graph)
# see help(nodeRenderInfo) for attributes.
#6 6
a04NEL0 <- as_graphNEL_sprof(sprof04, weight=FALSE)
#nDD0 <- (nodeRenderInfo(a04NEL0))
#a04NEL0 <- layoutGraph(a04NEL0)
#nDD1 <- (nodeRenderInfo(a04NEL0))# fill & col corrupted
nodeDataDefaults(a04NEL0, "shape") <- "ellipse"
#nodeDataDefaults(a04NEL0, "cex") <- 1.0
#nodeDataDefaults(a04NEL0, "weigth") <- 1
#nodeDataDefaults(a04NEL0, "fill") <- "green"
#nodeDataDefaults(a04NEL0, "col") <- "yellow"
...
legend("bottomright",legend= c("node color: self.time, pwr=0.5", 
	...
	legend("topleft", 
		legend=paste0("class: ",class(a04NEL0)),
		bg="#FFFFE040",
		seg.len=0)
...
\begin{figure}[htbp]
\begin{center}
\includegraphics{sprofiling-sprofadj04NEL02}
...
#6
#plot(sprof_igraph, main="sprof04: igraph layout", cex.main=2)
