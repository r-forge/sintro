...
See \vref{fig:sprofadj01NEL} for a comparison before and after trimming. The scaffold effect are now removed from the picture. You can do additional trimming, if you want.
...
We know ho to create standard graphh displays from this. The next step is to use additional information
we have from the profiles as attribute to the graph. The derived edge frequency is the first bit of information Implicitely, it can be used as weight in the graph placement routines. We make this explicit by giving a choice whether to use  it or not. Irrespective of this choice, we encode it as line width of the edges.
...
@
We still are experimentign with the facilities to display this graph.
...
\section{More Graphs}
\label{sec:moregraph}
\emph{Note: This section is collecting experiments  with various graph packages}
