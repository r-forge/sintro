...
%figure for help
<<fig = TRUE, echo = FALSE, print = FALSE, include=FALSE, label=barplots, png=TRUE, width=6, height=9>>= 
oldpar <- par(mfrow=c(3,2))
x <- runif(32)
barplot_s(x)
barplot_s(x, decreasing=FALSE)
barplot_s(x, lowtrim=0.1, hightrim=0.8)
barplot_s(x, lowtrim=0.1, hightrim=0.8, colfun="grey")
...
\label{subsec:sprof}
In contrast to the common \R{} profiling packages, in the \ircode{sprof} implementation we take a two step approach. First we read in the profile file to an internal representation. Analysis is done in later steps.
...
to \vref{tab:prSRREbt}). One difference is that \ircode{sprof} uses 
an event count as base, usually sampled by milliseconds (ms), whereas \R{} in general uses seconds as a base. Another difference is in two additional variables provided by \ircode{sprof}, the number of runs \ircode{nr\_runs} and  the average run length \ircode{avg\_time}. These are discussed in \vref{sec:rle}.
...
The node information is kept as a \ircode{data.frame}, and this can be 
extended as long as the overall structure is preserved. Two variables, 
\ircode{icol} and \ircode{ilwd} are by convention used to hold graphical 
...
have colour information, \ircode{icol} is to be used as an index into a colour 
table. If a display has a linewidth attribute, \ircode{ilwd} should be used to set 
the line width where feasible. The details are left to the plots.

Keeping in mind the limits of perception, \ircode{icol} and \ircode{ilwd} should 
...
For two common models, attributes by time and attributes by run length information, a helper routine is provided to calculate indices.\todo{make call by reference function}
<<label=funnodeattrs>>=
...
Note: we chose a colour palette for the full data set. If we restrict a display to 
...
{plot\_nodes}, and most plotting functions of \ircode{sprof}, allow various 
modes to control the use of colours. The recommended mode is to add a 
index variable \ircode{icol} giving an index to a colour table. See \irfunx{misc}
{rkindex} for a choice of modifications. By default, the colour table is the 
current palette. The plot functions allow to chose a different colour function 
plot by plot.\todo{add general discussion of colours in \ircode{sprof}}
<<>>=
...
\ircode{icol} is reserved for this) and use this as an index to a choice of 
colour palettes. So \ircode{icol} becomes a part of the data structure, and the 
...
On the node level, the additional information provided by \ircode{sprof} is the 
information on runs. The total time is broken down to runs. Detailed 
information is accessible if requested. The nuber of runs, and the average 
...
average run lenght (\vref{fig:run}: Run length information.
...
In \ircode{sprof}, we are making explicit use of this possibility.

The attribute \ircode{icol} is a special case which we used above. If present, 
...
%: floating figure
<<fig = TRUE, echo = TRUE, print = FALSE, include=FALSE, label=sREplotnodes01packcol1, png=TRUE, height=18, width=12>>= 
oldpar <- par(mfrow=c(3,2))
...
Nodes by class, user defined colours: see \vref{fig:sREplotnodes01col}.
...
\caption{8 12 Nodes by class. User defined colours.}
...
special stacks. In our example, \ircode{lm} or \ircode{summary.lm} may be convinient markers.
...
From the summary information, \ircode{nr\_runs} and \ircode{avg\_time} are
...
@
As \ircode{as\_graphNEL\_sprof}, the following function is not included in the package to avoid 
dependency of \ircode{sprof} on \ircode{graph} and other graph packages.
...
\ircode{plot\_graphNEL\_sprof( x, nodeattrs = "runs")}
...
Our example is very simple. In out example, we just use two functions, 
\ircode{lm} and \ircode{summary}. We should not be surprised to find them 
at prominent position with dominating weight. The \ircode{lm} and 
\ircode{summary.lm} are clearly separated and can be inspected separately.

In the \ircode{lm} branch, \ircode{eval} is prominent. In the overall structure 
of \R{},  \ircode{eval}  is the central interpreter. So it is expected to be 
...
\ircode{.getXlevels} is surprising. We have a plain vanilla real numbers 
problem. Why should it occur at all, and why at a prominent place? It is 
inside \ircode{lm}. But since \R{} is open source, we can inspect it. We find 
\ircode{.getXlevels} at exactly one place near the end of the source of 
\ircode{lm}:\\
\ircode{z\$xlevels <- .getXlevels(mt, mf)}\\
\ircode{z} is returned as result, and the help file tells us:\\
\emph{\ircode{xlevels}$\qquad$	
(only where relevant) a record of the levels of the factors used in fitting.}\\
In a pure regression problem, it is not relevant. But there is no conditional 
code at this place. \ircode{xlevels} is always calculated (at the cost of some 
...
\ircode{layout.norm(layout, 
	xmin = NULL, xmax = NULL, ymin = NULL, ymax = NULL,
      	zmin = NULL, zmax = NULL)}
@
The output of \ircode{igraph} gives problems when rendered with Adobe Acrobat on OS X Maverick. These examples have been moved to the \ircode{demo} section.
You can call them using\\
$\text{\hspace{3em}\ircode{demo(topic=<Item>, package = "sprof")}.}$
...
# 6 6
plot_graphNEL_sprof(sprof02,  layoutType="dot", nodeattrs="runs")
@
