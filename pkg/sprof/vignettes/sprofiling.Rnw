...
This package is now available from Bioconductor only, see \url{http://
...
foot approach. A more sophisticated function implementation is in 
\vref{sec:graph}.
...
xwhere <- function(oldseq, x){
	...
	while (firstpos <= lastpos) {
		if ( isTRUE(all.equal(oldseq , x[firstpos:(firstpos+l)])) )
			{return(firstpos) }
		firstpos <- firstpos+1
		}
	return(0)
...
xreplacenodes <- function(sprof, oldseq, newseq)
{
	...
	sapply(stacks, function(x){xreplace(oldseq, newseq, x)})
}
<<>>=
...
prxt(amt,
	 caption=paste0("Marginal statistics on nodes by run length, ",
		 "sorted by total time used,  count > 1 only"), 
	label="tab:pramt4", 
