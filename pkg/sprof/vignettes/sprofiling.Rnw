...
if (is.null(main)) 
	main <-paste0("plotviz( ",deparse(substitute(x)),", ", y, " ) ") else
	main=paste0(main, "\n plotviz( ",deparse(substitute(x)),", ", y, " ) ")
...
plot(x=x,  y=y,
	...
	legend("topleft", 
		legend=c(paste0("class: ",class(x)),paste0("method: ",y)),
		bg="#FFFFE040",
		seg.len=0
		)
}
...
plotviz(as(adjacency(sprof02),"graphNEL"),  
	sub=sprof02$info$id)
plotviz(as(adjacency(sprof04),"graphNEL"),  
	 sub=as.character(sprof04$info$id))
par(oldpar)
@
\begin{figure}[htbp]
\begin{center}
...
<<>>=
sprofedgel02 <- edgematrix(sprofadj02)
...
plotviz(sprofadjNEL02,  sub=sprof$info$id)
detach("package:graph")
@
\filledpagebreak
...
plot_network_sprof <- function( nwsprof, 
	mode = "fruchtermanreingold", main=NULL, label=NULL,...) {
	classnwsprof <- class(nwsprof)
	main1 <- deparse(substitute(nwsprof))
	if (!is.null(main)) main1 <- paste0(main,"\n",main1)
	
	if (inherits(nwsprof, "sprof")) nwsprof <- 	
		as_network_sprof(nwsprof)
		
	if (!is.null(label)) warning("explicit label supplied, but will use vertex names")
	
	plot( nwsprof,
		label = network.vertex.names(nwsprof), 
		main= paste0(main1, ": network layout\n ", mode), 
		mode = mode,
		edge.len=2,
		edge.col="#80808080",
		sub= main1,
