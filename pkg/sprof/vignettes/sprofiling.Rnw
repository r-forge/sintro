...
We can add attributes to the plots. But we can also attributes to the nodes, and use these in the plots. The attribute \ircode{icol} is a special case which we used above. If present, it will be interpreted as an index to a colour table. For example, we can collect special well known functions in groups.
...
\todo{keep as factor. This is a cube with margins node, stack level, run length.}
...
@
This is a poor first attempt to tame \ircode{profile_nodes_rlet}.
...
for (lev in (1:maxlevel) ) {
	...
	if (!is.null(proflev)) {
		...
		maxl <- max(proflev$length, na.rm=TRUE)
		if (maxl>maxlen) maxlen <- maxl
	}
}
## collapse profile_nodes_rle to 3d array
profile_nodes_rlearray <- array(0, 
	dim=c(maxnode,length(profile_nodes_rle), maxlevel), 
	dimnames= c("node", "level", "run_length"))
	
for (lev in (1:maxlevel) ) {
	proflev <- profile_nodes_rle[[lev]]
	if (!is.null(proflev)) {
		for (j in (1: length(proflev))){
