...
<<>>=
freqrank01 <- as_rkindex(-sprof01$nodes$total.time, ties.method="random")
...
el04 <- edgematrix(a04)
el04$lwd  <- as_rkindex(el04$count, maxindex=6, ties.method="min")
...
fill_list <- heat.colors(12)[as_rkindex(-sprof04$nodes$self.time, pwr=0.5, maxindex=12, ties.method="min")]
names(fill_list) <- sprof04$nodes$name

#sprof04$nodes$total.time -> lwd
lwd_list  <-  as_rkindex(sprof04$nodes$total.time, pwr=0.5,maxindex=6, ties.method="min")
...
# node attributes
#sprof04$nodes$self.time -> fill
fill_list <- heat.colors(12)[as_rkindex(-amt04[,"avg_time"], pwr=0.5, maxindex=12, ties.method="min")]
names(fill_list) <- sprof04$nodes$name

#sprof04$nodes$total.time -> lwd
lwd_list  <-  as_rkindex(amt04[,"nr_runs"], pwr=0.5,maxindex=6, ties.method="min")
names(lwd_list) <- sprof04$nodes$name
...
sprofedgel$lwd  <- as_rkindex(sprofedgel$count, maxindex=12, ties.method="min")
@
\filledpagebreak
...
edge.lwd<-sprofadj
edge.lwd[edge.lwd>0]<- as_rkindex(edge.lwd[edge.lwd>0], maxindex=12, ties.method="min")
...
#8
edge.lwd<-sprofadj
edge.lwd[edge.lwd>0]<- as_rkindex(edge.lwd[edge.lwd>0], maxindex=12,
 ties.method="max")
