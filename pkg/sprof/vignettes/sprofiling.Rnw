...
%\VignetteIndexEntry{Profile analysis for R}
%\VignetteKeyword{profile, visualisation}
\listfiles
...
\label{subsec:sprof}
In contrast to the common \R{} profiling packages, in the \code{sprof} 
implementation we take a two step approach. First we read in the profile file 
to an internal representation. Analysis is done in later steps.

This is a bottle neck to provide a point to use other data sets. The data we use for now is:
<<readRprof>>=
sprof01xx<- readRprof("RprofsRegressionExpl.out", id="lm")
@
<<sprof01xx>>=
sprof01<- sprof01xx
@
...
prxt(amt,
	 caption=paste0("Marginal statistics on nodes by run length, ",
		 "sorted by total time used (table trimmed)"), 
	label="tab:pramt4", 
...
<<fig = TRUE, echo = TRUE, print = FALSE, label=Rdplotsprof, png=TRUE, resolution=100, width=12, height=16, include=FALSE>>= 
#12 16
