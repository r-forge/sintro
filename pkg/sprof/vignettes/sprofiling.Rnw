...
Plot, for example, currently gives a choice of four displays for nodes, and supports trimming by default. Our profile starts with 62 nodes. The defaults cut off 34 nodes as uninformative, either because they are to rare, or ubiquitous.
 
<<fig=TRUE, label=sREplotnodes01, width=10, height=10>>=
#10
...
<<fig=TRUE, label=sREplotnodes01freqcol, width=10, height=10>>=
#10
...
{where <- getAnywhere(node)$where
	...
	if (is.null(where)) return("<??>") else
		...
		{print(paste(node,"not found. got:",where)); return("nn")}
		}
}
...
table(nodepackges)
#nodepackges <- lapply(as.character(sprof01$nodes$name),nodepackage0)
@
<<>>=
...
\begin{figure}[htbp]
\begin{center}
<<fig = TRUE, echo = FALSE, print = FALSE, label=sREplotnodes01packcol1, width=10, height=15>>= 
...
plot_nodes(sprof01, col=colpack)
plot(table(sprof01$nodes$icol),  
	type="h", lwd=20,col=colpack, lend="square", ylab="count")
nodescloud(sprof01, min.freq=5,  col=colpack)
par(oldpar)
@
...
See \vref{fig:sREplotnodes01col}.
...
#xtable(summary(sprof02))
#summary(sprof02)
str(sprof02$info)
<<print=FALSE, results=tex, label=tab:sprof02info2>>=
print(xtable(sprof02$nodes, caption="sprof02, after update", 
	label="tab:sprof02info2"),floating=FALSE, 
		tabular.environment="longtable", 
		caption.placement="top",
		NA.string="\\vdots")
@
...
str(profile_nodes_rlearray, max.level=2, vec.len=3,nchar.max=40, list.len=6)
...
library(graph)
plot(as(adjacency(sprof01),"graphNEL"),  
	main="graph layout sprof01", cex.main=2)
plot(as(adjacency(sprof02),"graphNEL"),  
	main="graph layout sprof02", cex.main=2)
par(oldpar)
@
...
plot(sprofadjRag, main="sprof01: Graphviz dot layout", cex.main=2)
@
...
#8
plot(sprofadjRag,"neato", main="sprof01: Graphviz neato layout", cex.main=2)
@
\filledpagebreak
<<fig=TRUE, label=sprofadjviztwopi,width=8, height=8>>=
#6
plot(sprofadjRag,"twopi", main="sprof01: Graphviz twopi layout", cex.main=2)
@
\filledpagebreak
<<fig=TRUE, label=sprofadjvizcirco,width=8, height=8>>=
plot(sprofadjRag,"circo", main="sprof01: Graphviz circo layout", cex.main=2)
@
\filledpagebreak
<<fig=TRUE, label=sprofadjvizfdp,width=8, height=8>>=
plot(sprofadjRag,"fdp", main="sprof01: Graphviz fdp layout", cex.main=2)
@
\filledpagebreak
...
library(igraph)
sprofig <- graph.adjacency(sprofadj)
<<fig=TRUE, label=s03ig,width=6, height=6>>=
#6
#plot(sprofig, main="sprof03: igraph layout", cex.main=2)
plot(sprofig, main="sprof03: igraph layout: trimmed data")
detach("package:igraph")
@
\filledpagebreak
\subsubsection{network package}
<<fig=TRUE, label=s03nwsprofadj,width=6, height=6>>=
#6
library(network)
nwsprofadj <- as.network(sprofadj) # names is not imported 
network.vertex.names(nwsprofadj) <- rownames(sprofadj) # not honoured by plot
plot(nwsprofadj, label=rownames(sprofadj), 
	main="sprof03: network layout: trimmed data", cex.main=2)
@
\filledpagebreak
...
edge.lwd <- round(edge.lwd/max(edge.lwd)*12)
plot(nwsprofadj, label=rownames(sprofadj), 
	main="sprof03: network layout: trimmed data", 
...
library(Rgraphviz)
sprofadjRag <- agopen(sprofadjNEL, name="Rprof Example")
<<fig=TRUE, label=s03adjvizdot, width=6, height=6>>=
#6
plot(sprofadjRag, main="sprof03: Graphviz dot layout", cex.main=2)
@
%agopen(graph, name, nodes, edges, kind = NULL, layout = TRUE,
%layoutType = "dot",
%attrs = list(), nodeAttrs = list(), edgeAttrs = list(),
...
%dot, neato, twopi, circo, and fdp.
\filledpagebreak
<<fig=TRUE, label=s03adjvizneato,width=6, height=6>>=
#6
plot(sprofadjRag,"neato", main="sprof03: Graphviz neato layout", 
	cex.main=2)
@
\filledpagebreak
<<fig=TRUE, label=s03adjviztwopi,width=6, height=6>>=
#6
plot(sprofadjRag,"twopi", main="sprof03: Graphviz twopi layout")
@
\filledpagebreak
<<fig=TRUE, label=sprofadjvizcirco,width=6, height=6>>=
#6
plot(sprofadjRag,"circo", main="sprof03: Graphviz circo layout", cex.main=2)
@
\filledpagebreak
<<fig=TRUE, label=s03adjvizfdp,width=6, height=6>>=
#6
plot(sprofadjRag,"fdp", main="sprof03: Graphviz fdp layout", cex.main=2)
@
\index{Index01}
%:Sweave examples
%<<print=TRUE>>=
